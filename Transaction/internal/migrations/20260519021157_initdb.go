...
func init() {
	goose.AddNamedMigrationContext("20260519021157_initdb.go", upCreateTransactionTable, downCreateTransactionTable)
}
...
func downCreateTransactionTable(ctx context.Context, tx *sql.Tx) error {
	queries := []string{
		...
		`DROP INDEX idx_transaction_entries_account_id ON transactions_entries;`,
